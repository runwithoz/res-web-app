$(function () {
  ...
  function showNext() {
    ...
    const next = current.next("p").length
      ? current.next("p")
      : $(".carousel>p").first();
    current.css({"z-index":"100"}).stop().animate({width:0},500, "linear").removeClass("active");
    next.css({"width": "700px", "z-index":"0"}).fadeIn(1000).addClass("active");
  ...
  function showPrev() {
    ...
    const prev = current.prev("p").length
      ...
      : $(".carousel>p").last();
      current.css({"z-index":"100"}).stop().animate({width:0},500, "linear").removeClass("active");
      prev.css({"width": "700px", "z-index":"0"}).fadeIn(1000).addClass("active");
  }
